...
global $fioxen_job_preview;
$fioxen_job_preview = $form->get_job_id();
...
<form method="post" id="job_preview" action="<?php echo esc_url( $form->get_action() ); ?>">
	...
	<div class="job_listing_preview_title clearfix">
		<input type="submit" name="continue" id="job_preview_submit_button" class="button job-manager-button-submit-listing" value="<?php echo esc_attr( apply_filters( 'submit_job_step_preview_submit_text', __( 'Submit Listing', 'fioxen' ) ) ); ?>" />
		<input type="submit" name="edit_job" class="button job-manager-button-edit-listing" value="<?php esc_attr_e( 'Edit listing', 'fioxen' ); ?>" />
		<h2><?php esc_html_e( 'Preview', 'fioxen' ); ?></h2>
	</div>
